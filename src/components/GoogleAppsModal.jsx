...
const AppIcon = ({ type, name }) => {
    switch (type) {
        case 'search':
            return (
                <div className="w-full h-full rounded-[22%] bg-white flex items-center justify-center shadow-xs">
        ...
        case 'gemini':
            return (
                <div className="w-full h-full rounded-[22%] bg-gradient-to-br from-[#1b1a55] via-[#535c91] to-[#070f2b] p-1.5 flex items-center justify-center shadow-xs border border-indigo-400/30">
        ...
        case 'youtube':
            return (
                <div className="w-full h-full rounded-[22%] bg-[#FF0000] flex items-center justify-center shadow-xs">
        ...
        case 'calendar':
            return (
                <div className="w-full h-full rounded-[22%] bg-white flex flex-col items-center justify-center shadow-xs overflow-hidden border border-neutral-200">
                    <div className="w-full bg-[#1a73e8] text-[8px] font-bold text-white uppercase text-center py-0.5 leading-none">
                        {new Date().toLocaleString('en-US', { month: 'short' })}
                    </div>
                    <div className="flex-1 flex items-center justify-center text-neutral-800 font-bold text-base font-mono">
                        {new Date().getDate()}
                    </div>
                </div>
            );
        case 'maps':
            return (
                <div className="w-full h-full rounded-[22%] bg-[#4285F4] flex items-center justify-center shadow-xs overflow-hidden relative">
                    <div className="absolute inset-0 bg-gradient-to-tr from-[#34A853] via-[#FBBC04] to-[#4285F4] opacity-80"></div>
                    <svg className="w-6 h-6 relative z-10 drop-shadow-xs" viewBox="0 0 24 24">
        ...
        case 'docs':
            return (
                <div className="w-full h-full rounded-[22%] bg-[#4285F4] p-2 flex flex-col justify-between shadow-xs">
                    <div className="w-full h-1 bg-white/90 rounded-full"></div>
                    <div className="w-3/4 h-1 bg-white/70 rounded-full"></div>
        ...
        case 'sheets':
            return (
                <div className="w-full h-full rounded-[22%] bg-[#0F9D58] p-2 flex flex-col justify-center shadow-xs">
        ...
        case 'slides':
            return (
                <div className="w-full h-full rounded-[22%] bg-[#F4B400] p-2 flex items-center justify-center shadow-xs">
                    <div className="w-full h-full border-2 border-white/90 rounded-xs flex items-center justify-center bg-white/15">
                        <div className="w-2.5 h-1.5 bg-white rounded-xs"></div>
                    </div>
                </div>
            );
        case 'keep':
            return (
                <div className="w-full h-full rounded-[22%] bg-[#FBBC04] flex items-center justify-center shadow-xs">
                    <svg className="w-7 h-7" viewBox="0 0 24 24" fill="white">
        ...
        case 'translate':
            return (
                <div className="w-full h-full rounded-[22%] bg-gradient-to-br from-[#4285F4] to-[#1a73e8] flex items-center justify-center shadow-xs">
                    <span className="text-white font-serif font-bold text-base">文A</span>
                </div>
            );
        case 'news':
            return (
                <div className="w-full h-full rounded-[22%] bg-white flex flex-col items-center justify-center shadow-xs p-1.5">
                    <div className="w-full h-2 bg-[#4285F4] rounded-xs mb-1"></div>
                    <div className="w-full h-0.5 bg-neutral-400 rounded-full mb-0.5"></div>
                    <div className="w-full h-0.5 bg-neutral-300 rounded-full"></div>
                </div>
            );
        case 'contacts':
            return (
                <div className="w-full h-full rounded-[22%] bg-gradient-to-br from-[#1a73e8] to-[#4285F4] flex items-center justify-center shadow-xs">
                    <svg className="w-7 h-7" viewBox="0 0 24 24" fill="white">
        ...
        case 'chrome':
            return (
                <div className="w-full h-full rounded-[22%] bg-white flex items-center justify-center shadow-xs overflow-hidden">
        ...
        case 'cloud':
            return (
                <div className="w-full h-full rounded-[22%] bg-[#1a73e8] flex items-center justify-center shadow-xs">
                    <svg className="w-7 h-7" viewBox="0 0 24 24" fill="white">
        ...
        case 'colab':
            return (
                <div className="w-full h-full rounded-[22%] bg-neutral-900 border border-amber-500/40 flex items-center justify-center shadow-xs">
                    <span className="text-amber-400 font-black text-base font-mono">co</span>
                </div>
            );
        default:
            return (
                <div className="w-full h-full rounded-[22%] bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-xs text-white font-bold text-base">
                    {name?.charAt(0) || 'G'}
                </div>
            );
    }
...
const GOOGLE_APPS = [
    ...
    { id: 'cloud', name: 'Cloud', category: 'ai_dev', type: 'cloud', url: 'https://console.cloud.google.com' },
];
...
const GoogleAppsModal = ({ isOpen, onClose }) => {
    ...
    return (
        <div
            className="fixed inset-0 z-50 bg-black/40 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in select-none"
            onClick={onClose}
        >
            {/* Perfectly Sized macOS Translucent Glass Launchpad Window */}
            <div
                ref={modalRef}
                onClick={e => e.stopPropagation()}
                className="w-full max-w-2xl h-[490px] rounded-[24px] p-5 shadow-[0_25px_60px_rgba(0,0,0,0.5)] flex flex-col relative overflow-hidden border border-white/20 animate-pop-in"
                style={{
            ...
            >
                ...
                <div className="flex items-center justify-between pb-3 border-b border-white/10 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-1.5 ml-1">
                            ...
                            <h2 className="text-sm font-semibold text-white tracking-tight">Applications</h2>
                        </div>
                    </div>
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            autoFocus
                            className="w-36 focus:w-48 bg-white/10 border border-white/10 rounded-full pl-7 pr-3 py-1 text-xs text-white placeholder-white/40 focus:outline-none focus:bg-white/15 focus:border-white/30 transition-all font-sans"
                        />
                    </div>
                </div>

                {/* 2. macOS Category Filter Pills */}
                <div className="flex items-center gap-1.5 py-2.5 overflow-x-auto custom-scrollbar shrink-0">
                    {GOOGLE_CATEGORIES.map(cat => (
                        <button
                            key={cat.id}
                            onClick={() => setActiveCat(cat.id)}
                            className={`px-3 py-0.5 rounded-full text-[11px] font-medium transition-all shrink-0 ${activeCat === cat.id
                ...
                {/* 3. Compact App Squircle Grid */}
                <div className="grid grid-cols-5 sm:grid-cols-6 gap-y-3.5 gap-x-2 py-2 overflow-y-auto custom-scrollbar flex-1 items-start pr-1">
                    {filteredApps.map(app => (
                        ...
                        >
                            {/* Compact Squircle */}
                            <div className="w-12 h-12 rounded-[22%] transition-all group-hover:scale-105 group-hover:shadow-[0_8px_18px_rgba(0,0,0,0.4)] p-0.5">
                                <AppIcon type={app.type} name={app.name} />
                            </div>

                            {/* Label */}
                            <span className="text-[10.5px] font-medium text-white/85 group-hover:text-white text-center tracking-tight leading-tight mt-1 max-w-[70px] truncate drop-shadow-xs">
                                {app.name}
                            </span>
                        </a>
                    ))}
                    ...
                    {filteredApps.length === 0 && (
                        <div className="col-span-full py-12 text-center text-xs text-white/40 font-sans">
                ...
                {/* 4. Bottom Status */}
                <div className="pt-2.5 border-t border-white/10 flex items-center justify-between text-[10.5px] text-white/40 font-sans shrink-0">
                    <span>Google Apps Launchpad</span>
